...
bootstrap <- function(data, iter = 1000, seed = 123, ...){
  ...
  risks <- function(data, ...){
    ...
    fit <- glm(outcome ~  trt + z1 +z2+ z3+ z4 + x1 + x2 + x3 + x4,
...
causal.or <- function(risks, boot,  ...){
  ...
  Y_0_b <- sapply(boot, function(x) x["0"]) #function(x) tryCatch(x["0"], error = function(e) NA)  
  Y_1_b <- sapply(boot, function(x) x["1"])
  
  ## count number of failures and remove NA
  fail <- length(which(is.na(Y_0_b)))
  if(! fail == 0){
    Y_0_b <- as.numeric(Y_0_b[-which(is.na(Y_0_b))])
