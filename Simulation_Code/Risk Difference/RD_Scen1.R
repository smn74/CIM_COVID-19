...
ps_methods <- function(dt, ...){
  ...
  # ps model includes all observed variables 
  ...
  lm.adj <- lm(outcome ~ trt + x1 + x2 + x3 + x4, data = dt)
  ...
  ## 4.) Matching
  ps.mod <- tryCatch(matchit(trt ~ x1+ x2+x3+x4, data = dt,
  ...
  boots <- bootstrap(dt, form = outcome ~  trt + z + x1 + x2 + x3 + x4)
  ...
  boots2 <- bootstrap(dt, form = outcome ~  trt + z1 + z2 + z3 + z4 +  x1 + x2 + x3 + x4)
