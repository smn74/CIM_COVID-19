# Code taken from PSW
ps.aug <- function(dt, form.ps, weight = "ATE",form.out, family="binomial"){
  ...
  psw.aug.core <- function(dat, beta.hat, 
    ...
    Amat <- Bmat <- 0
    for (i in 1:n) {
      Xi <- as.numeric(c(1, dat[i, .(x1, x2 ,x3, x4, x5, x6, x7, x8, x9)]))
      Vi <- as.numeric(c(1, dat[i, .(x1, x2 ,x3, x4, x5, x6, x7, x8, x9)]))
      Zi <- dat[i, trt]
      Yi <- dat[i, outcome]
      Wi <- W[i]
      Qi <- Q[i]
