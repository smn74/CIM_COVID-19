# Code taken from PSW
ps.aug <- function( data, form.ps, weight = "ATE",form.outcome, family="binomial"){

  # clinicalstatus must be dummy coded
  data[, clinicalstatus.1 := ifelse(clinicalstatus == "URTI", 1, 0)]
  data[, clinicalstatus.2 := ifelse(clinicalstatus == "LRTI", 1, 0)]
  
  out.ps <- PSW:::ps.model(dat = data, form.ps = as.formula(form.ps))
  ...
  omega <- sapply(ps.hat, PSW:::calc.omega, weight = weight, delta = 0.002, 
                  K = K)
  Q <- data[, trt] * ps.hat + (1 - data[, trt]) * (1 - ps.hat)
  W <- omega/Q
  res <- list(weight = weight, ps.model = out.ps$fm, ps.hat = ps.hat, 
              W = W)
  out.outcome <- PSW:::outcome.model(dat = data, form = as.formula(form.out), 
  ...
  psw.aug.core <- function(dat, beta.hat, 
    ...
    Amat <- Bmat <- 0
    for (i in 1:n) {
      Xi <- as.numeric(c(1, dat[i, .(sex, age, clinicalstatus.1, clinicalstatus.2, timeonset)]))
      Vi <- as.numeric(c(1, dat[i, .(sex, age, clinicalstatus.1, clinicalstatus.2, timeonset)]))
      Zi <- dat[i, trt]
      Yi <- dat[i, outcome]
      Wi <- W[i]
      Qi <- Q[i]
  ...
  res.aug <- psw.aug.core(dat = data, beta.hat = beta.hat, 
